...
function generateDailyUpdate({ topic = "daily", preferredCharacter = null } = {}) {
  let pool = characters;

  if (preferredCharacter) {
    pool = characters.filter(c => c.name === preferredCharacter);
  } else if (topic === "food") {
    pool = characters.filter(c => c.activity.includes("便當") || c.activity.includes("布丁"));
  } else if (topic === "performance") {
    pool = characters.filter(c => c.activity.includes("高音") || c.activity.includes("舞台"));
  } else if (topic === "trouble") {
    pool = characters.filter(c => c.reaction || c.joinResult);
  }
  ...
  } else if (main.name === "杏奈" || main.name === "昴") {
    sentence += joinType === "success"
